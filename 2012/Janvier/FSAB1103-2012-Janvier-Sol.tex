...
\solution{
  \begin{enumerate}
    ...
    \item
      ...
      \begin{align*}
        \frac{r^2R''+rR'}{R} & = -\frac{\Theta ''}{\Theta} =  k^2 & \text{car condition homogène en } \theta.
      \end{align*}
      \paragraph{En $\Theta$ :}
      $$ \Theta '' + k^2 \Theta = 0$$
      La solution générale de cette équation est : $\Theta(\theta) = A \cos(k\theta) + B \sin(k\theta)$.\\
      On calcule : $\Theta '(0) = B k = 0 \Longleftrightarrow B = 0$.\\
      ...
      La solution générale de cette équation est : $R_n = C_nr^n+D_nr^{-n}$. Comme nous avons une solution finie en r=0 ($u(0,\theta) = u_0$), le terme $r^{-n}$ ne convient pas ici. Nous obtenons donc : 
      \begin{align*}
